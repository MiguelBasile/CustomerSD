...
USER_CUSTOMER_MAP=miguel.basile@fivetwo.nz=fivetwo,janicebasile@fivetwo.nz=fivetwo
...
For production customer preview, map each signed-in Entra user to the ADO customer value they are allowed to see:

```env
USER_CUSTOMER_MAP=miguel.basile@fivetwo.nz=fivetwo,janicebasile@fivetwo.nz=fivetwo
```

The left side is the signed-in user UPN/email. The right side must exactly match the value stored in `ADO_CUSTOMER_FIELD` for that customer's work items. When a user has a mapping, the dashboard does not ask for a pasted customer token. When `USER_CUSTOMER_MAP` is configured, signed-in users without a mapping are blocked from ticket data.

The older token fallback remains for local testing or temporary preview access. The token format is:
...
or paste it into the dashboard token field when no `USER_CUSTOMER_MAP` entry exists for the signed-in user. The dashboard stores the token in browser local storage for fallback preview access and does not put it in ticket URLs.
...
Then paste the generated customer token into the dashboard when mock mode is off and no user-customer mapping is configured.
...
- `GET /api/session` reads the Static Web Apps signed-in principal and reports whether the user is in `ALLOWED_USER_UPNS` and mapped by `USER_CUSTOMER_MAP`.
...
- Configure Static Web App application settings for `ADO_ORG`, `ADO_PROJECT`, `ADO_AUTH_MODE=entra`, `ADO_ENTRA_TENANT_ID`, `ADO_ENTRA_CLIENT_ID`, `ADO_ENTRA_CLIENT_SECRET`, `ADO_ENTRA_SCOPE`, `ADO_CUSTOMER_FIELD=Custom.Customer`, `ADO_WORK_ITEM_TYPES`, `ALLOWED_USER_UPNS`, `USER_CUSTOMER_MAP`, `CUSTOMER_TOKEN_SECRET`, and `MOCK_MODE=false`.
...
- `/api/*` remains reachable at the SWA layer because the Functions validate the signed-in user allowlist and customer mapping before returning customer-scoped DTOs.

This adds an Entra gate in front of the static dashboard. Customer scoping comes from `USER_CUSTOMER_MAP`; the customer-token path remains as a fallback only when no user-customer map is configured.
...
Set `USER_CUSTOMER_MAP` to comma-separated `user=customer` pairs. For example, `miguel.basile@fivetwo.nz=fivetwo` lets that signed-in user load only work items whose `Custom.Customer` value is `fivetwo`.
